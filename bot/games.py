...
async def _on_cricket_pick(query, context, cid, ch, user, n: int) -> None:
    ...
    if ch["status"] != "live":
        await query.answer("Accept the challenge first.", show_alert=True)
        return
    status = record_pick(ch, user.id, n)
    if status == "dup":
        other = ch["b"] if user.id == ch["a"] else ch["a"]
        other_n = await _short_name(context, ch["chat_id"], other)
        await query.answer(f"Already locked. Waiting for {other_n}.")
        return
    _save_match(cid, ch)
...
async def _resolve_cricket_ball(query, context, cid: str, ch: dict) -> None:
    ...
    _save_match(cid, ch)
    if phase == "finished":
        await _finish_cricket(query, context, cid, ch, ch.get("last") or "Match over.")
        return
    if phase == "chase":
        await query.answer(ch.get("waiting") or "Chase begins.")
    else:
        await query.answer(ch.get("last") or "Ball done.")
    await _show_cricket(query, context, cid, ch, markup=_hand_markup(cid))
